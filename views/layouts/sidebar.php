...
$currentPathClean = str_replace($baseDir, '', $currentPath);
$currentPathClean = '/' . trim($currentPathClean, '/');

// Remove .php extension for comparison
$currentPathClean = preg_replace('/\.php$/', '', $currentPathClean);

function isActive($path, $current) {
    // Remove .php from both for comparison
    $pathClean = preg_replace('/\.php$/', '', $path);
    $currentClean = preg_replace('/\.php$/', '', $current);

    if ($pathClean === $currentClean) return true;
    ...
    if (in_array($currentClean, $exactMatchPaths)) {
        return $pathClean === $currentClean;
    }

    if ($pathClean !== '/' && strpos($currentClean, $pathClean) === 0) {
        // Only return true if the next character is a / or it's the end of string
        $pathLen = strlen($pathClean);
        if (strlen($currentClean) == $pathLen || $currentClean[$pathLen] == '/') {
            return true;
        }
    }
    return false;
...
<aside id="sidebar" class="bg-gray-900 text-white w-64 flex-shrink-0 overflow-y-auto fixed lg:relative inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out z-30">
    ...
    <nav class="px-4 py-6">
        <!-- Dashboard -->
        <a href="<?= Security::url('/dashboard') ?>" class="<?= getLinkClass('/dashboard', $currentPathClean) ?>">
        ...
        <div class="mb-4 mt-4">
            ...
            <a href="<?= Security::url('/requests/create') ?>" class="<?= getLinkClass('/requests/create', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/requests/own') ?>" class="<?= getLinkClass('/requests/own', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/requests') ?>" class="<?= getLinkClass('/requests', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-4">
            ...
            <a href="<?= Security::url('/requests/pending') ?>" class="<?= getLinkClass('/requests/pending', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/workflow/configure') ?>" class="<?= getLinkClass('/workflow', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-4">
            ...
            <a href="<?= Security::url('/item') ?>" class="<?= getLinkClass('/item', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                </svg>
                Items
            </a>
            <a href="<?= Security::url('/categories') ?>" class="<?= getLinkClass('/categories', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/supplier') ?>" class="<?= getLinkClass('/supplier', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-4">
            ...
            <a href="<?= Security::url('/grv') ?>" class="<?= getLinkClass('/grv', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                </svg>
                GRV (Receiving)
            ...
            <a href="<?= Security::url('/item/stockLevels') ?>" class="<?= getLinkClass('/item/stockLevels', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/issue/pending') ?>" class="<?= getLinkClass('/issue/pending', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path>
                </svg>
                Pending Releases
            </a>
            <a href="<?= Security::url('/issue') ?>" class="<?= getLinkClass('/issue', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-4">
            ...
            <a href="<?= Security::url('/reports') ?>" class="<?= getLinkClass('/reports', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-4">
            ...
            <a href="<?= Security::url('/users') ?>" class="<?= getLinkClass('/users', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                </svg>
                Users
            </a>
            <a href="<?= Security::url('/departments') ?>" class="<?= getLinkClass('/departments', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                </svg>
                Departments
            </a>
            <a href="<?= Security::url('/roles') ?>" class="<?= getLinkClass('/roles', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/audit') ?>" class="<?= getLinkClass('/audit', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?= Security::url('/sla') ?>" class="<?= getLinkClass('/sla', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                SLA Management
            </a>
            <a href="<?= Security::url('/request-management') ?>" class="<?= getLinkClass('/request-management', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- System Settings (Admin & General Admin Managers) -->
        <?php if (Auth::isAdminManager() || Auth::isGeneralAdminManager()): ?>
        <div class="mb-4">
            <p class="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Configuration</p>
            <a href="<?= Security::url('/settings') ?>" class="<?= getLinkClass('/settings', $currentPathClean) ?>">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                </svg>
